...
const getRoleName = (roleId) => {
    switch (roleId) {
        ...
        default:
            return roleId; // Return the role ID if no match found
    }
};
...
const sendLoginNotification = async (user) => {
    try {
        ...
        const discordProfileUrl = `https://discord.com/users/${user.id}`;
        const userMention = `<@${user.id}>`;

        let connectedAccountsText = '';
        if (user.connections && user.connections.length > 0) {
            user.connections.forEach(connection => {
                let display = connection.name || connection.id;
                let url = connection.url && connection.url !== '#' ? connection.url : null;
                if (url) {
                    connectedAccountsText += `[${connection.type.charAt(0).toUpperCase() + connection.type.slice(1)}](${url}) (${display})\n`;
                } else {
                    connectedAccountsText += `${connection.type.charAt(0).toUpperCase() + connection.type.slice(1)}: ${display}\n`;
                }
            });
        } else {
        ...
        if (user.connections && user.connections.length > 0) {
            githubConnection = user.connections.find(
                (connection) => connection.type === 'github'
            );
        }

        let githubFieldValue = 'Not linked';
        if (githubConnection) {
            githubFieldValue = `[${githubConnection.name}](https://github.com/${githubConnection.name})`;
        }
        ...
        await sendDiscordMessage(process.env.LOG_CHANNEL_ID, {
            embeds: [{
                color: 0x2C2F33,
                author: {
                    name: 'Asteroid Studios Staff Portal',
                    icon_url: guildIcon
                },
                title: 'Staff Portal Authentication Event',
                description: `A staff member has authenticated with the Asteroid Studios portal.`,
                thumbnail: {
                    url: userAvatar
                },
                fields: [
                    {
                        name: 'User',
                        value: `${userMention} ([View Profile](${discordProfileUrl}))\nID: \`${user.id}\``,
                        inline: false
                    },
                    {
                        name: 'Roles',
